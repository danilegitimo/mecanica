@extends('layouts.app')

@section('title', 'Peças')

@section('header.title', 'Peças')

@section('header.subtitle', 'Todos as peças cadastradas e disponíveis para consulta no banco de dados.')

@section('header.content')
<div>
  <a href="{{ route('parts.create') }}" class="btn btn-success">Cadastrar</a>
</div>
@endsection

@section('content')
<div>
  @if ( $parts->isNotEmpty() )
  <div class="search">
    <form action="{{ route('parts.index') }}" class="d-flex">
      <input name="search" type="search" class="form-control me-2" placeholder="Pesquisar..." value="{{ Request::has('search') ? Request::input('search') : '' }}" />
      <button class="btn btn-success">Pesquisar</button>
    </form>
  </div>
  <table class="default-table">
    <thead>
      <tr>
        <th>Nome</th>
        <th>Quantidade</th>
        <th>Fornecedor</th>
        <th>Ações</th>
      </tr>
    </thead>
    <tbody>
      @foreach ($parts as $part)
      <tr>
        <td>
          <p class="m-0 p-0" style="font-size: 10px;">{{ $part->id }}</p>
          <p class="m-0 p-0 fw-bolder">{{ $part->name }}</p>
        </td>
        <td>
          {{ $part->quantity }}/pcs
        </td>
        <td>
          <p class="m-0 p-0" style="font-size: 14px;">{{ $part->fornecedor->cnpj }}</p>
          <p class="m-0 p-0 fw-bolder">{{ $part->fornecedor->name }}</p>
        </td>
        <td>
          <a href="{{ route('parts.edit', $part->id) }}" class="btn btn-success">Editar</a>
          <form action="{{ route('parts.destroy', $part->id) }}" method="POST" style="display:inline">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger">Excluir</button>
          </form>
        </td>
      </tr>
      @endforeach
    </tbody>
  </table>
  <div class="m-4">
    {{ $parts->links() }}
  </div>
  @else
    @if ( Request::has('search') )
    <div class="empty-data p-4 d-flex flex-column justify-content-center align-items-center">
      <p>A sua pesquisa não retornou nenhum dado para o termo: <span class="fw-bolder">{{ Request::input('search') }}</span></p>
      <a href="{{ route('parts.index') }}" class="btn btn-success">Voltar</a>
    </div>
    @else
    <div class="empty-data p-4 d-flex flex-column justify-content-center align-items-center">
      <p>Poxa! Não há nada para mostrar. Que tal cadastrar?</p>
      <a href="{{ route('parts.create') }}" class="btn btn-success">Cadastrar</a>
    </div>
    @endif
  @endif
</div>
@endsection